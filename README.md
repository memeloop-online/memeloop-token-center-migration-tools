...
这是一个公开可复用、一次性、隔离于生产请求数据面的迁移工具仓库。它提供严格的 TypeScript 工具、声明式 SQL 输入、合成测试夹具、一个最小 Go SQLite 封存命令与操作说明，用于在一次经过审批的迁移窗口内导出、校验、导入、回放和对账。
...
- 运维脚本使用 TypeScript；SQL 只作为声明式迁移输入。唯一的 Go 命令 `cpa-session-archive-backup` 使用 SQLite online-backup API 生成一次性封存副本，并由 CI 构建、测试和发布。
...
最初产品源码提取的精确来源、Apache-2.0 许可和逐文件 SHA-256 见 [`SOURCE-MANIFEST.json`](SOURCE-MANIFEST.json)。SQLite 封存命令来自另一公开仓库，经过有意的性能与可观测性修改，其独立来源声明见 [`cmd/cpa-session-archive-backup/SOURCE.md`](cmd/cpa-session-archive-backup/SOURCE.md)。
...
npm run typecheck
npm test
go test ./cmd/cpa-session-archive-backup
```

大体量 PostgreSQL acceptance 默认跳过，只有显式提供隔离 schema 的测试环境变量时才会执行；退役对象清理契约由普通 CI 提供隔离 PostgreSQL，并与 SQLite 路径成对验证。Release CI 为 archive 引擎提供隔离 PostgreSQL并执行 SQLite/PostgreSQL 导入契约，同时使用锁定 Go module 构建并测试 SQLite 封存命令。`ops/import-cpa-session-archive.ts` 使用固定 product revision 构建并随 Release 封存的 Rust 二进制；这是交付中间步骤，并不表示历史源码闭包已提取或允许清理。本地不安装依赖、不拉取源码、不接触真实源/目标数据；本地验证仅限 `git diff --check`。

## 固定 revision 的 Release 执行模型

正式 TypeScript 迁移命令使用 CI 为完整 Git revision 构建并验证的 GitHub Release 包，而不是可变标签或 npm 包。SQLite 封存 Job 使用 CI 从同一 revision 发布的静态、digest-addressable OCI 运行时。每个 Release 的 tag 为 `migration-tools-<40-char-git-sha>`，并附带 `release-manifest.json` 与 `SHA256SUMS`；CI 不覆写已有 tag 的资产。迁移记录必须保存 revision、Release tag、包 SHA-256、备份镜像 digest 和非敏感 evidence digest。
