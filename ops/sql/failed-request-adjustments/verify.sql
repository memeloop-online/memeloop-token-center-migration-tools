...
WITH selected_tenant AS MATERIALIZED (
...
), affected_request_daily_dimensions AS MATERIALIZED (
...
), affected_analysis_hourly_dimensions AS MATERIALIZED (
  SELECT DISTINCT f.tenant_id, f.key_id, f.created_at / 3600000 AS hour_bucket, f.model,
    CASE WHEN f.protocol='anthropic' OR f.protocol LIKE 'anthropic-%' THEN 'anthropic' WHEN f.protocol='openai-image' THEN 'openai-image' ELSE 'openai' END AS protocol,
    f.status_class, f.error_code, f.upstream_account_id, f.model_route_id, f.service_tier, f.currency
  FROM request_stats_facts f JOIN items i ON i.request_id=f.request_id AND i.tenant_id=f.tenant_id
), affected_analysis_daily_dimensions AS MATERIALIZED (
  SELECT DISTINCT f.tenant_id, f.key_id, f.created_at / 86400000 AS day_bucket, f.model,
    CASE WHEN f.protocol='anthropic' OR f.protocol LIKE 'anthropic-%' THEN 'anthropic' WHEN f.protocol='openai-image' THEN 'openai-image' ELSE 'openai' END AS protocol,
    f.status_class, f.error_code, f.upstream_account_id, f.model_route_id, f.service_tier, f.currency
  FROM request_stats_facts f JOIN items i ON i.request_id=f.request_id AND i.tenant_id=f.tenant_id
), rollup_mismatches AS MATERIALIZED (
  SELECT d.tenant_id, d.key_id, d.day_bucket, d.model FROM affected_request_daily_dimensions d
  ...
  UNION ALL
  SELECT d.tenant_id, d.key_id, d.hour_bucket, d.model FROM affected_analysis_hourly_dimensions d
  LEFT JOIN usage_analysis_hourly h ON h.tenant_id=d.tenant_id AND h.key_id=d.key_id AND h.hour_bucket=d.hour_bucket AND h.source_kind='request' AND h.model=d.model AND h.protocol=d.protocol AND h.status_class=d.status_class AND h.error_code=d.error_code AND h.upstream_account_id=d.upstream_account_id AND h.model_route_id=d.model_route_id AND h.service_tier=d.service_tier AND h.currency=d.currency
  WHERE h.cost_micros IS DISTINCT FROM (SELECT sum(f.cost_micros-COALESCE(i.refund_micros,0)) FROM request_stats_facts f LEFT JOIN items i ON i.request_id=f.request_id AND i.tenant_id=f.tenant_id WHERE f.tenant_id=d.tenant_id AND f.key_id=d.key_id AND f.created_at/3600000=d.hour_bucket AND f.model=d.model AND (CASE WHEN f.protocol='anthropic' OR f.protocol LIKE 'anthropic-%' THEN 'anthropic' WHEN f.protocol='openai-image' THEN 'openai-image' ELSE 'openai' END)=d.protocol AND f.status_class=d.status_class AND f.error_code=d.error_code AND f.upstream_account_id=d.upstream_account_id AND f.model_route_id=d.model_route_id AND f.service_tier=d.service_tier AND f.currency=d.currency)
  UNION ALL
  SELECT d.tenant_id, d.key_id, d.day_bucket, d.model FROM affected_analysis_daily_dimensions d
  LEFT JOIN usage_analysis_daily a ON a.tenant_id=d.tenant_id AND a.key_id=d.key_id AND a.day_bucket=d.day_bucket AND a.source_kind='request' AND a.model=d.model AND a.protocol=d.protocol AND a.status_class=d.status_class AND a.error_code=d.error_code AND a.upstream_account_id=d.upstream_account_id AND a.model_route_id=d.model_route_id AND a.service_tier=d.service_tier AND a.currency=d.currency
  WHERE a.cost_micros IS DISTINCT FROM (SELECT sum(f.cost_micros-COALESCE(i.refund_micros,0)) FROM request_stats_facts f LEFT JOIN items i ON i.request_id=f.request_id AND i.tenant_id=f.tenant_id WHERE f.tenant_id=d.tenant_id AND f.key_id=d.key_id AND f.created_at/86400000=d.day_bucket AND f.model=d.model AND (CASE WHEN f.protocol='anthropic' OR f.protocol LIKE 'anthropic-%' THEN 'anthropic' WHEN f.protocol='openai-image' THEN 'openai-image' ELSE 'openai' END)=d.protocol AND f.status_class=d.status_class AND f.error_code=d.error_code AND f.upstream_account_id=d.upstream_account_id AND f.model_route_id=d.model_route_id AND f.service_tier=d.service_tier AND f.currency=d.currency)
